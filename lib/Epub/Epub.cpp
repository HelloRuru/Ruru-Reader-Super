...
bool Epub::parseContentOpf(BookMetadataCache::BookMetadata& bookMetadata, const bool parseSpine) {
...
Serial.printf(" [EBP] 最大连续内部RAM块: %dKB | 剩余总RAM: %dKB", heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL)/1024, heap_caps_get_free_size(MALLOC_CAP_INTERNAL)/1024);
  ...
  ContentOpfParser opfParser(getCachePath(), getBasePath(), contentOpfSize, parseSpine ? bookMetadataCache.get() : nullptr);
  if (!opfParser.setup()) {
    Serial.printf("[%lu] [EBP] Could not setup content.opf parser\n", millis());
    return false;
  }
  Serial.printf("[%lu] [EBP] here1 \n", millis());

  if (!readItemContentsToStream(contentOpfFilePath, opfParser, 1024)) {
    Serial.printf("[%lu] [EBP] Could not read content.opf\n", millis());
    return false;
  }
  Serial.printf("[%lu] [EBP] here2 \n", millis());
...
bool Epub::load(const bool buildIfMissing, const bool skipLoadingCss) {
  ...
  const bool effectiveSkipLoadingCss = skipLoadingCss || kDisableEpubCss;

  if (kDisableEpubCss) {
    Serial.printf("[%lu] [EBP] CSS parsing is disabled by configuration\n", millis());
  }
  ...
  if (bookMetadataCache->load()) {
    ...
    if (!effectiveSkipLoadingCss && !loadCssRulesFromCache()) {
      ...
      if (!parseContentOpf(bookMetadataCache->coreMetadata, false)) {
