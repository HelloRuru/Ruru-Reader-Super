...
| 項目 | 大小 |
| --- | --- |
| Flash usage | `4,998,666 / 6,553,600 bytes`，約 `76.3%` |
| RAM usage | `108,916 / 327,680 bytes`，約 `33.2%` |
| `firmware.bin` | 約 `5,191,856 bytes` |
...
| UI 字型子集化 | 可用 | build 前自動依常用中文字、介面文字與符號重新產生 10px UI/外部文字字型 |
...
3. 產生 `tools/font-subset/ui_charset.txt` 與 `ui_charset_intervals.txt`。
4. 從 `tools/font-subset/ubuntu_10_bold.full.h.xz` 取出必要 glyph。
5. 重寫 `lib/EpdFont/builtinFonts/ubuntu_10_bold.h`。

目前 UI/外部文字 10px 字型子集為 `6861 glyphs`、`4287 intervals`、`352,962 bitmap bytes`。這比純 UI 子集大，但仍小於完整 CJK 字型。
...
`ubuntu_10_bold.h` 是 UI 與外部標題/檔名用的 10px 常用字集字型，不是書籍內文的完整補字字型。書籍內容主要依閱讀字型與 `notosans_18_bold` 等內建字型處理。
